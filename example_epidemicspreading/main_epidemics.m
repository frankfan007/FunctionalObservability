...
%   Last modified by Arthur Montanari on 09/10/2020
...
% is described by the connections between only 81 cities.
load airnet.mat
...
%% Epidemic spreading model (Eq. 8, Ref. [1])

% Nodal dynamics are described by a SIRD model (Eq. 8, Ref. [1]) with the
...
%% Structural model of the epidemic spreading model in Eq. 8, Ref. [1]
...
% between states in the ODEs in Eq. 8, Ref. [1]. In other words, the
...
T = targetcities + N;            % target states (i.e., target state vars.
                                 % corresponding to the infected states I_i
                                 % of target cities)
F = sparse(1:r, T, 1, r, n);     % functional matrix

%% Minimum sensor placement
Cand = nodesample(r+1:N);        % targets cannot be sensor candidates
Cand = Cand + 3*N;               % only dead states D_i can be candidates
S = find_msp(Astruct,T,Cand);    % minimum sensor placement
...
% functional observer, stated in Eq. (S-20), designed *specifically* for
...
% False predictions. We simulate the epidemiological model in Eq. 8 with
% false initial conditions (different from the above) to contrast results
% where the prediction is run freely (with no functional observer) and
% where the prediction is given by a functional observer.
cityoutbreak_false = datasample(1:N,1); % one city is chosen randomly as
...
% False prediction of the epidemic spreading (Free simulation)
[~,x_false] = odeRK(@(t,s)SIRDnet(t,s,N,P,Adj,beta,gamma,eta),...
    [t0 dt tf], x0_false');
x_false = x_false';

% Prediction with the aid of a functional observer and measurements of the
% sensor cities
y = C*x;                        % output vector
w = zeros(size(foD,2),length(tspan));       % initialization
w(:,1) = inv(foD)*(F0*x0_false - foE*C*x0); % initialized w/false assump.
% zhat = zeros(r0,T);           % initialization
...
% happens in each city with the epidemic peaks predicted with and without
% the aid of a functional observer (and measurements from sensor cities).
...
% False predictions (without the aid of a functional observer)
z_false = F*x_false;
for i = 1:r
    [~,arg] = max(z_false(i,:));
    tp_false(1,i) = t(arg);
...
% Predictions with the aid of a functional observer
% False predictions (without the aid of a functional observer)
for i = 1:r
    [~,arg] = max(zhat(i,:));
    tp_fobsv(1,i) = t(arg);
...
disp(['True value --- Free simulation prediction --- Prediction with functional observer'])
results_abs = [tp' tp_false' tp_fobsv']

disp(['Difference between the true time instant of the epidemic peak and the predictions per target city:'])
disp(['Free simulation prediction --- Prediction with functional observer'])
results_dif = [abs(results_abs(:,1) - results_abs(:,2)) abs(results_abs(:,1) - results_abs(:,3))]

disp(['Average performance of the prediction methods over all target cities'])
disp(['Free simulation prediction --- Prediction with functional observer'])
...
disp(['Note that predictions made based on free simulation are very',...
    ...
    ' outbream and thus can achieve better or close performance (to',...
    ' the simulations made with functional observers)',...
    ' in a few specific cases.'])

save test_data_v3.mat
