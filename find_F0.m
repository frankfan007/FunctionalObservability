...
% implementation of Algorithm 2 in Ref. [1]. See Ref. [1] for more details.
%
% Inputs:
...
A(:,[T; S]) = 0;
G = C*A;             % G is only composed by rows of CA, since C and F
...
while cond_while == 0
    ...
    % If false, F0 needs to be augmented.
        ...
        % node (drawn element from the set C) is removed since their
...
% set of nodes where each element corresponds to a row of G, X is a set of
% state nodes where each element also corresponds to a column of G, and
% (v_i,x_j) is an undirected edge in E if G(i,j)~=0.
...
% in this row, then the x_j \in X is a right-matched node, i.e.,
% x_j \in M_1. Thus, sensor and target nodes are always right-matched
