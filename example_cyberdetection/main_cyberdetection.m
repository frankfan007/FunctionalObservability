...
%% Defines the linear dynamical matrix A 
...
% PMUs are randomly placed on 30% of the generator terminals
% and load buses. 
...
% states" T desired to be estimated by the functional observers.
...
% states are not used by a functional observer.
S = S(S~=TS);
...
Neigh = Neigh(Neigh>ng) + ng;  % fake data cannot be taken from generators
                               % phase
...
% System operators in steady-state until t=1s 
...
Nobsv = 10;               % number of functional observers to be designed
...
plot(tspan(1:step:end),x(Fake,1:step:end),'-',...
    'Color',[0.788, 0.149, 0.156],'LineWidth',3)    % false data by attack
plot(tspan(1:step:end),zhatdash(1:5:end,1:step:end),'--',...
